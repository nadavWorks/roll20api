...
const dldark = (() => {
    ...
    const buildCircleGrid = function (who, rad, left, top, makeGrid, gridSize) {
        ...
        try {
            ...
            createdPaths.push({ path: circlePoints, left: left, top: top, width: rad * 2, height: rad * 2, rotation: 0 });
            ...
            if (makeGrid) {
                ...
                for (x = startX; x <= 2 * rad; x += gridSize) {
                    if (x >= 0) {
                        //vertical lines
                        lineSize = 2 * Math.sqrt(rad * rad - (x - centerX) * (x - centerX));
                        ...
                        let verticalPath = JSON.stringify([['M', 0, lineSize], ['L', 0, 0]]);
                        createdPaths.push({ path: verticalPath, left: left - centerX + x, top: top, width: 0, height: lineSize, rotation: 0 });
                    }
                }
                ...
                for (y = startY; y <= 2 * rad; y += gridSize) {
                    if (y >= 0) {
                        //horizontal lines
                        lineSize = 2 * Math.sqrt(rad * rad - (y - centerY) * (y - centerY));
                        ...
                        let horizontalPath = JSON.stringify([['M', lineSize, 0], ['L', 0, 0]]);
                        createdPaths.push({ path: horizontalPath, left: left, top: top - centerY + y, width: lineSize, height: 0, rotation: 0 });
                    }
                }
            }
    ...
    const buildWallGrid = function (who, tok, buffer) {
        let createdPaths = [];
        try {
            let left = tok.get("left");
            let top = tok.get("top");
            let width = tok.get("width");
            let height = tok.get("height");
            let rotation = tok.get("rotation");
            let lineSize = (Math.min(width, height) / 2) + buffer;
            let wallSize = (Math.max(width, height) / 2) + buffer;
            let convergenceDistance = wallSize - lineSize;

            // top-left
            let topLeftPath = JSON.stringify([['M', lineSize, lineSize], ['L', 0, 0]]);
            let topLeftPathPosition = rotate(left - (width / 2 + buffer) + lineSize / 2, top - (height / 2 + buffer) + lineSize / 2, left, top, rotation);
            createdPaths.push({ path: topLeftPath, left: topLeftPathPosition.x, top: topLeftPathPosition.y, width: lineSize, height: lineSize, rotation: rotation });

            // top-right
            let topRightPath = JSON.stringify([['M', 0, lineSize], ['L', lineSize, 0]]);
            let topRightPathPosition = rotate(left + (width / 2 + buffer) - lineSize / 2, top - (height / 2 + buffer) + lineSize / 2, left, top, rotation);
            createdPaths.push({ path: topRightPath, left: topRightPathPosition.x, top: topRightPathPosition.y, width: lineSize, height: lineSize, rotation: rotation });

            // bottom-left
            let bottomLeftPath = JSON.stringify([['M', 0, lineSize], ['L', lineSize, 0]]);
            let bottomLeftPathPosition = rotate(left - (width / 2 + buffer) + lineSize / 2, top + (height / 2 + buffer) - lineSize / 2, left, top, rotation);
            createdPaths.push({ path: bottomLeftPath, left: bottomLeftPathPosition.x, top: bottomLeftPathPosition.y, width: lineSize, height: lineSize, rotation: rotation });

            // bottom-right
            let bottomRightPath = JSON.stringify([['M', lineSize, lineSize], ['L', 0, 0]]);
            let bottomRightPathPosition = rotate(left + (width / 2 + buffer) - lineSize / 2, top + (height / 2 + buffer) - lineSize / 2, left, top, rotation);
            createdPaths.push({ path: bottomRightPath, left: bottomRightPathPosition.x, top: bottomRightPathPosition.y, width: lineSize, height: lineSize, rotation: rotation });

            if (width < height) {
                // vertical
                let verticalPath = JSON.stringify([['M', 0, convergenceDistance * 2], ['L', 0, 0]]);
                createdPaths.push({ path: verticalPath, left: left, top: top, width: 0, height: 2 * convergenceDistance, rotation: rotation });

            } else if (width > height) {
                // horizontal
                let horizontalPath = JSON.stringify([['M', convergenceDistance * 2, 0], ['L', 0, 0]]);
                createdPaths.push({ path: horizontalPath, left: left, top: top, width: 2 * convergenceDistance, height: 0, rotation: rotation });
            }
    ...
    const rotate = function (pointX, pointY, AxisX, AxisY, degrees) {
        let radians = degrees * Math.PI / 180;
        let newX = (pointX - AxisX) * Math.cos(radians) - (pointY - AxisY) * Math.sin(radians) + AxisX;
        let newY = (pointX - AxisX) * Math.sin(radians) + (pointY - AxisY) * Math.cos(radians) + AxisY;
        return {
            x: newX,
            y: newY
        };
    ...
    const handleInput = function (msg) {
        ...
        try {
            ...
            if (msg.type == "api" && msg.content.indexOf("!dldark") == 0) {
                ...
                if (cmd.length > 1) {
                    if (isNaN(cmd[1])) {
                        sendChat(scriptName, `/w "${who}" ` + 'Non-numeric buffer detected. Format is \"!dldark # boolean boolean type\"');
                        return;
                    } else {
                ...
                if (cmd.length > 2) {
                    if (_.contains(['true', 'yes', '1'], cmd[2].toLowerCase())) {
                        ...
                        sendChat(scriptName, `/w "${who}" ` + 'Non-boolean makeGrid flag detected. Format is \"!dldark # boolean boolean type\"');
                        return;
                    }
                }
                if (cmd.length > 3) {
                    if (_.contains(['true', 'yes', '1'], cmd[3].toLowerCase())) {
                        ...
                        sendChat(scriptName, `/w "${who}" ` + 'Non-boolean sendToMapLayer flag detected. Format is \"!dldark # boolean boolean type\"');
                        return;
                    }
                }
                if (cmd.length > 4) {
                    if (_.contains(['circle', 'radial'], cmd[4].toLowerCase())) {
                        isCircle = true;
                    } else if (_.contains(['wall', 'skeleton'], cmd[4].toLowerCase())) {
                        isCircle = false;
                    } else {
                        sendChat(scriptName, `/w "${who}" ` + 'unknown darkness type detected. Format is \"!dldark # boolean boolean type\"');
                        return;
                    }
                }
                ...
                if (isCircle) {
                    ...
                    creathedPaths = buildCircleGrid(who, radius, left, top, makeGrid, pageGridSize);
                } else {
                    creathedPaths = buildWallGrid(who, tok, buffer);
                }

                for (let i = 0; i < creathedPaths.length; i++) {
                    let path = createObj("path", {
                        pageid: tok.get("pageid"),
                        path: creathedPaths[i].path,
                        fill: "transparent",
                        stroke: "#ff0000",
                        layer: "walls",
                        //stroke_width: 5,
                        ...
                        //left: tok.get("left"),
                        //top: tok.get("top"),
                        width: creathedPaths[i].width,
                        height: creathedPaths[i].height,
                        left: creathedPaths[i].left,
                        top: creathedPaths[i].top,
                        rotation: creathedPaths[i].rotation
                    });
